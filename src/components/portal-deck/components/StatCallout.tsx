...
import { accentStyle, balancedTextStyle, bodyTextStyle, containerQueryStyle, labelTextStyle, metricValueTextStyle, safeTextStyle } from "./utils";
  ...
  return (
    <section className={cn("deck-two-panel min-w-0 gap-8 rounded-md border border-border bg-card-background p-8", className)} style={{ ...accentStyle(accent), ...containerQueryStyle }}>
      <div className="min-w-0 self-end" style={containerQueryStyle}>
        ...
        <div className="mt-4 min-w-0 font-semibold tabular-nums" style={{ ...metricValueTextStyle(density, density === "dense" ? 3 : 4.6), color: "var(--deck-accent)" }}>{value}</div>
      </div>
