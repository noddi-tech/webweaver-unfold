...
export function PortalComponentsPreview() {
  ...
  const componentRenders = useMemo(() => [
    { name: "Hero", render: (density: Density) => <Hero density={density} variant={density === "sparse" ? "gradient" : "minimal"} eyebrow="Navio investorpresentasjon" title="En fot innenfor hos verdens største merkevarer" subtitle="Navio hjelper bilglass-, dekk- og flåteoperatører med å gjøre fragmentert mobil etterspørsel om til repeterbar by-for-by drift." metrics={samples.metrics} kicker="NOK 10–20 mill. seed-utvidelse" /> },
    ...
    { name: "CategoryCard", render: (density: Density) => <div className="deck-auto-grid gap-4"><CategoryCard density={density} label="Vertikal 01" title="Bilglass" description="Mobile bilglassoperatører trenger booking, ruting, feltutførelse og kundeoppdateringer i ett operativt lag." status="Bevist i drift" metric="Hurtigruta Carglass" /><CategoryCard density={density} label="Vertikal 02" title="Dekktjeneste" description="Sesongtopper gjør rutedensitet og kapasitetsplanlegging særlig verdifullt." status="Replikeringsmål" metric="NOK 10–20 mill. emisjon" /><CategoryCard density={density} label="Vertikal 03" title="Flåtevedlikehold" description="Gjentakende B2B-etterspørsel gir vei til forutsigbar utnyttelse på tvers av byklynger." status="Ekspansjonsmulighet" metric="Operatørpublikum" /></div> },
  ], [samples]);
  ...
  const visualSlides = useMemo(() => [
    ...
    sampleSlide("verticals", "vertikaler", "Vertikaler", { items: [{ name: "Bilglass", status: "Bevist i drift", description: "Mobil tjenesteutførelse med høy rutedensitet." }, { name: "Dekk", status: "Neste vertikal", description: "Sesongtopper gir tydelig behov for kapasitetsstyring." }] }),
