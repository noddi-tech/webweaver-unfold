...
export function InvestorDetail() {
  ...
  return <PortalCmsLayout title="Investor detail" description={email}>
    ...
    <Collapsible open={open} onOpenChange={setOpen}><Card><CardHeader><CollapsibleTrigger asChild><Button variant="outline">{open ? "Skjul" : "Vis"} rå hendelseslogg</Button></CollapsibleTrigger></CardHeader><CollapsibleContent><CardContent><Table><TableHeader><TableRow><TableHead>Type</TableHead><TableHead>Path</TableHead><TableHead>Payload</TableHead><TableHead>Dwell</TableHead><TableHead>Created</TableHead></TableRow></TableHeader><TableBody>{paged.map((e) => <TableRow key={e.id}><TableCell>{e.event_type}</TableCell><TableCell>{e.path ?? "—"}</TableCell><TableCell><pre className="max-w-md overflow-auto text-xs">{JSON.stringify(e.payload, null, 2)}</pre></TableCell><TableCell>{formatDwell(e.dwell_seconds)}</TableCell><TableCell>{formatDate(e.created_at)}</TableCell></TableRow>)}</TableBody></Table><div className="mt-4 flex gap-2"><Button variant="outline" disabled={page === 0} onClick={() => setPage(page - 1)}>Forrige</Button><Button variant="outline" disabled={(page + 1) * 25 >= events.length} onClick={() => setPage(page + 1)}>Neste</Button></div></CardContent></CollapsibleContent></Card></Collapsible>
  </PortalCmsLayout>;
}
