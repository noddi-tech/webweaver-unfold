...
const RATE_LIMIT_MESSAGE_NB = "Du har nådd grensen på 30 AI-forespørsler per time (genereringer og endringer telles sammen).";
const VALID_VISUAL_TYPES = new Set([
  "cover", "logos", "badges", "funnel", "adoption", "glide",
  "team", "round", "gap", "verticals", "customer-spotlight", "custom",
]);
...
type RefineRequest = {
  slug?: unknown;
  parent_draft_id?: unknown;
  current_state?: unknown;
  instruction?: unknown;
  include_style_references?: unknown;
};
...
type ValidationResult = { ok: true; value: RefineResponse } | { ok: false; errors: string[] };
...
const DESIGN_PRINCIPLES = `DESIGN PRINCIPLES (apply rigorously):
...
const SYSTEM_PROMPT = `You are iterating on a slide for Navio Solutions' investor pitch deck with the editor in a back-and-forth conversation. The editor has reviewed the current draft and provided an instruction. Your job is to apply their instruction.

INSTRUCTION TYPES:

The editor's instruction may be:

  Surgical — small changes to specific text or values.
    Examples: 'Change verksteder to tjenester'
              'Drop the word faktisk'
              'Make the headline shorter'
    Response: change only what was asked. Preserve everything else.

  Directional — changes to the slide's structure, approach, or feel.
    Examples: 'Make this feel more like Apple — sparser, drop the body'
              'Try a custom composition with Hero and StatGrid'
              'Reframe this for an investor who doesn\\'t know the industry'
              'Switch to a SectionDivider treatment with gradient'
    Response: redesign the slide to match the direction. You may change visual_type if the direction calls for it. You may restructure visual_config completely. The new slide should feel like a fresh draft following the editor's direction, not a patch of the old one.

  Mixed — both surgical and directional.
    Examples: 'Make it more like Apple AND change verksteder to tjenester'
    Response: apply both. Surgical changes preserved through the restructure.

DO NOT mechanically classify the instruction — interpret it. If the editor says 'make this stronger' it's directional. If they say 'change X to Y' it's surgical. Use judgment.

CONSTRAINT: visual_type can change only to one of the suggested visual_types in the slide brief. If the editor's instruction implies a visual_type the brief doesn't allow, pick the closest allowed one and explain in ai_note.
...
Return JSON with this shape:
{
  ...
  "body_md": string (optional),
  "visual_type": string,
  "visual_config": object,
  "ai_note": string (optional, Norwegian Bokmål, max 200 chars — only for non-obvious choices like visual_type changes or major restructures; omit for self-explanatory surgical edits)
}
...
function validateRefineResponse(raw: unknown, suggested: string[]): ValidationResult {
  const errors: string[] = [];
  ...
  const aiNote = raw.ai_note;

  if (!isString(title) || title.trim().length < 1 || title.length > 80) errors.push("title is required and must be 1-80 characters");
  if (subtitle !== undefined && subtitle !== null && (!isString(subtitle) || subtitle.length > 120)) errors.push("subtitle must be a string up to 120 characters when provided");
  if (body !== undefined && body !== null && (!isString(body) || body.length > 4000)) errors.push("body_md must be a string up to 4000 characters when provided");
  ...
  if (aiNote !== undefined && aiNote !== null && (!isString(aiNote) || aiNote.length > 200)) errors.push("ai_note must be a string up to 200 characters when provided");

  if (errors.length) return { ok: false, errors };
  const note = isString(aiNote) && aiNote.trim() ? aiNote.trim() : undefined;
  return {
    ok: true,
    value: {
...
function validateCurrentState(raw: unknown): { ok: true; value: CurrentState } | { ok: false; error: string } {
  if (!isRecord(raw)) return { ok: false, error: "current_state must be an object" };
  const title = raw.title;
  const visualType = raw.visual_type;
  const visualConfig = raw.visual_config;
  if (!isString(title) || title.trim().length < 1) return { ok: false, error: "current_state.title is required" };
  if (!isString(visualType) || !VALID_VISUAL_TYPES.has(visualType)) return { ok: false, error: "current_state.visual_type is invalid" };
  if (!isRecord(visualConfig)) return { ok: false, error: "current_state.visual_config must be an object" };
  const subtitle = raw.subtitle;
  const bodyMd = raw.body_md;
  return {
    ok: true,
    value: {
      title: title.trim(),
      subtitle: isString(subtitle) ? subtitle : undefined,
      body_md: isString(bodyMd) ? bodyMd : undefined,
...
// Heuristic: detect if ai_note appears to be English rather than Norwegian Bokmål.
// Looks for common English-only function words. Norwegian shares many cognates,
// so we only flag when multiple unmistakably-English markers appear.
function detectLikelyEnglish(text: string): boolean {
  const lowered = ` ${text.toLowerCase().replace(/[^\p{L}\s]/gu, " ")} `;
  const englishMarkers = [" the ", " with ", " changed ", " switched ", " because ", " your ", " this ", " that ", " these ", " those ", " from ", " into ", " using "];
  let hits = 0;
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const parentDraftId = typeof body.parent_draft_id === "string" ? body.parent_draft_id.trim() : "";
    ...
    if (!parentDraftId || !isUuid(parentDraftId)) return json({ error: "parent_draft_id must be a UUID" }, 400);
    if (!instruction) return json({ error: "instruction is required" }, 400);
    if (instruction.length > 4000) return json({ error: "instruction must be at most 4000 characters" }, 400);

    const stateValidation = validateCurrentState(body.current_state);
    if (!stateValidation.ok) return json({ error: stateValidation.error }, 400);
    const currentState = stateValidation.value;

    // Shared rate-limit budget with draft-slide.
    ...
    if ((count ?? 0) >= MAX_DRAFTS_PER_HOUR) return json({ error: RATE_LIMIT_MESSAGE_NB }, 429);
    ...
    // Verify parent draft exists and belongs to this editor.
    const { data: parentRow, error: parentError } = await serviceClient
      .from("portal_slide_drafts")
      .select("id,editor_user_id,slide_slug")
      .eq("id", parentDraftId)
      .maybeSingle();
    if (parentError) throw parentError;
    if (!parentRow) return json({ error: "parent_draft_id not found" }, 404);
    if (parentRow.editor_user_id !== userData.user.id) {
      return json({ error: "parent_draft_id belongs to a different editor" }, 403);
    }
    if (parentRow.slide_slug !== slug) {
      return json({ error: "parent_draft_id does not belong to this slide" }, 400);
    }

    // Reference data (same scope as draft-slide; no per-call subset for refinements).
    const referencesToUse = typedBrief.reference_resources ?? [];
  ...
  Suggested visual types (visual_type must be one of these): ${JSON.stringify(typedBrief.suggested_visual_types)}

Current draft state (this is what the editor sees right now):
...
Visual config schemas for the allowed visual_types:
${JSON.stringify(visualSchemas, null, 2)}

Return only valid JSON in the OUTPUT FORMAT specified in the system prompt. Do not wrap in markdown.`;
    ...
    const validation = validateRefineResponse(parsed, typedBrief.suggested_visual_types);
    if (!validation.ok) return json({ errors: validation.errors }, 422);

    const validated = validation.value;
    const visualTypeChanged = validated.visual_type !== currentState.visual_type;

    // Soft language check on ai_note. Don't reject — just record a warning.
    const warnings: string[] = [];
    if (validated.ai_note && detectLikelyEnglish(validated.ai_note)) {
      warnings.push("ai_note appears to be English rather than Norwegian Bokmål");
      console.warn("refine-slide-draft: ai_note may be English:", validated.ai_note);
    }

    const promptContext = {
      system_prompt: SYSTEM_PROMPT,
      schema: visualSchemas,
      references_passed: referencesToUse,
      parent_state: currentState,
      instruction,
    ...
    return json({ id: insertedRow?.id ?? null, ...validated });
  } catch (error) {
    console.error("refine-slide-draft error:", error);
